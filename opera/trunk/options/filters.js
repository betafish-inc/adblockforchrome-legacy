...
function validateOverSubscription() {
  ...
  if (optionalSettings.show_advanced_options) {
    // In case of an advanced user, only warn once every session
    if (sessionStorage.getItem('noOversubscriptionWarning')) {
      return true;
    } else {
      sessionStorage.setItem('noOversubscriptionWarning', 'true');
    }
  }
