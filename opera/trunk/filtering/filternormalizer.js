...
var FilterNormalizer = {
  ...
  normalizeLine: function(filter) {
    ...
    if (Filter.isSelectorFilter(filter)) {
      ...
      // Opera can't handle regex rules. Remove the "real" regex rules here; the simple ones will be converted to strings later
      // Leave in the rules with $document $elemhide or $popup, they can still be parsed
      if (!(types & ~ElementTypes.DEFAULTTYPES) && /^(@@)?\/.*?[\\\.\*\{\}\+\?\^\$\[\]\(\)\|\<\>\#].*?\/(?:\$~?[\w\-]+(?:=[^,\s]+)?(?:,~?[\w\-]+(?:=[^,\s]+)?)*)?$/.test(filter)) {
        return null;
      }
    }
